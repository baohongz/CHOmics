		...
		echo '<div class="w-100 d-flex justify-content-start">';
			...
			if(! $BXFILES_READONLY){
				echo '<div class="ml-auto p-2">
					<button class="btn btn-sm btn-info mr-1" id="create_folder_btn"><i class="fas fa-plus-square"></i> New Folder</button>
					...
					<button class="btn btn-sm btn-primary mr-1" id="batch_upload_btn"><i class="fas fa-upload"></i> Batch Upload</button>
				</div>';
			}
			...
			echo '<div class="w-100 p-2">
				<table class="table table-sm table-hover dataTable">
					<thead>
						...
						if(! $BXFILES_READONLY) echo '<th class="text-center"><input type="checkbox" id="select_all_checkbox"></th>';
						else echo '<th class="text-center">No</th>';
				...
				foreach($folder_children_list as $key => $value){
					...
					if(is_dir($current_file)){
						...
						$file_url_link = 'Shared:' . substr($current_file, strlen($DESTINATION_SUBFOLDER_DIR) - 1) . '';
						...
						$file_url_link = '<a href="' . $BXAF_BXFILES_PREFIX . $current_file_encrytped . "&u=$BXAF_USER_CONTACT_ID" . '" target="_blank"><i class="fas fa-external-link-alt hidden-xl-up"></i> <span class="hidden-lg-down fix_width_font">Shared:' . substr($current_file, strlen($DESTINATION_SUBFOLDER_DIR) - 1) . '</span></a>';
					}

					echo '<tr>';

					if(! $BXFILES_READONLY) echo '<td class="text-center"><input type="checkbox" class="checkbox_row" filename="' . $name_encrytped . '"></td>';
					else echo '<td class="text-center">' . $n . '</td>';
