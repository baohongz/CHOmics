...
<nav class="navbar text-nowrap navbar-expand-md fixed-top <?php echo $BXAF_CONFIG['BXAF_PAGE_CSS_MENU']; ?>">
        ...
        <?php
        	...
        	if (isset($bxaf_menu_items) && is_array($bxaf_menu_items) && count($bxaf_menu_items) > 0){
        		...
        		foreach($bxaf_menu_items as $key=>$menu_item){
                    ...
                    $menu_item_class= '';
                    if(isset($menu_item['Class']) && $menu_item['Class'] != '') $menu_item_class = $menu_item['Class'];
        			...
        			if (! isset($menu_item['Children']) || ! is_array($menu_item['Children']) || count($menu_item['Children']) <= 0){
        				...
        				echo "<li class='nav-item {$active_class}'><a class='nav-link {$menu_item_class}' href='{$menu_item['URL']}'>{$icon}{$URL_Name}</a></li>";
        			}
        				...
        				echo "<li class='nav-item dropdown " . $active_class . "'>";
        					...
        					echo "<a class='nav-link dropdown-toggle {$menu_item_class}' href='#' id='bxaf_page_menu_$key' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>" . htmlentities($menu_item['Name'], ENT_QUOTES | ENT_IGNORE, "UTF-8") . "</a>";
        					...
        					foreach($menu_item['Children'] as $subkey=>$submenu_item){
                                ...
                                $submenu_item_class= '';
                                if(isset($submenu_item['Class']) && $submenu_item['Class'] != '') $submenu_item_class = $submenu_item['Class'];

        						$active_class = '';
        						if(array_key_exists($subkey, $active_subclass) ) $active_class = $active_subclass[$subkey];

                                if(isset($submenu_item['Class']) && $submenu_item['Class'] == 'divider'){
        							echo "<hr class='my-2' />";
        						}

        						else if(isset($submenu_item['URL']) && $submenu_item['URL'] != '' && isset($submenu_item['Name']) && $submenu_item['Name'] != ''){
        							echo "<a class='dropdown-item {$active_class} {$submenu_item_class}' href='" . $submenu_item['URL'] . "'><i class='fas fa-caret-right'></i> " . htmlentities($submenu_item['Name'], ENT_QUOTES | ENT_IGNORE, "UTF-8") . "</a>";
        						}

        						else if(isset($submenu_item['Name']) && $submenu_item['Name'] != ''){
        							echo "<a class='dropdown-item {$submenu_item_class}'>" . htmlentities($submenu_item['Name'], ENT_QUOTES | ENT_IGNORE, "UTF-8") . " </a>";
        						}

                                else {
        							echo "<a class='dropdown-item {$submenu_item_class}'></a>";
        						}
