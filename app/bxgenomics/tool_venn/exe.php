...
if(isset($_GET['action']) && $_GET['action']=="upload"){
	...
	//echo '<pre>'; print_r($value_from_csv); echo '</pre>';
...
if(isset($_GET['action']) && $_GET['action']=="draw_pic"){
	...
	if($_POST['sets']=='3' && $_POST['image_type']==2){
				...
				$n3_plot = $n3 * $coef;
				/*
				$text1x_plot = (($n1)/2) * $coef + 10;
				$text1y_plot = ($n3 + $o3y + ($n1)/2) * $coef + 10;
				$text2x_plot = ($n1 + $o2x + ($n2)/2) * $coef + 10;
				$text2y_plot = ($n3 + $o3y) * $coef + 10;
				$text3x_plot = $o3x_plot;
				$text3y_plot = ($n3)/2 * $coef + 10;
				$text12x_plot = ($n1 + $o2x - $n2) * $coef + 10 + 25;
				$text12y_plot = ($n3 + $o3y) * $coef + 10 + 35;
				$text23x_plot = ($n1 + $o3x - ($n3*($n2+$o2x-$o3x))/sqrt(pow($o3y,2)+(pow($n2+$o2x-$o3x,2)))) * $coef + 10;
				$text23y_plot = ($n3 + ($n3*$o3y)/sqrt(pow($o3y,2)+(pow($n1+$o3x,2)))) * $coef + 10 -25;
				$text13x_plot = ($n1 + $o3x + ($n3*($n1+$o3x))/sqrt(pow($o3y,2)+(pow($n1+$o3x,2)))) * $coef + 10;
				$text13y_plot = ($n3 + ($n3*$o3y)/sqrt(pow($o3y,2)+(pow($n1+$o3x,2)))) * $coef + 10 -25;
				*/
				$icon1y_plot = $o1y_plot + $n1 * $coef + 40;
				$icon2y_plot = $icon1y_plot + 20;
				$icon3y_plot = $icon1y_plot + 40;
