...
if(intval($_GET['id']) > 0){
    ...
    $COMPARISON_INFO = $BXAF_MODULE_CONN -> get_row($sql, intval($_GET['id']) );
    if (! is_array($COMPARISON_INFO) || count($COMPARISON_INFO) <= 0) {
        echo 'Error: No comparison found.';
        exit();
    }
    $COMPARISON_INDEX   = $COMPARISON_INFO['ID'];
    $COMPARISON_SPECIES = $COMPARISON_INFO['Species'];

    $direction = (array_key_exists('direction', $_GET) && $_GET['direction'] == 'Down') ? 'Down' : 'Up';

    $comparison_dir = $BXAF_CONFIG['GO_OUTPUT'][    strtoupper($COMPARISON_SPECIES)] . "comp_{$COMPARISON_INDEX}/comp_{$COMPARISON_INDEX}_GO_Analysis_{$direction}/";
    $comparison_url = $BXAF_CONFIG['GO_OUTPUT_URL'][strtoupper($COMPARISON_SPECIES)] . "comp_{$COMPARISON_INDEX}/comp_{$COMPARISON_INDEX}_GO_Analysis_{$direction}/";
}

// tool_functional_enrichment
else if (isset($_GET['comp_subdir']) && trim($_GET['comp_subdir']) != '') {
    $comp_subdir = $_GET['comp_subdir'];

    $comparison_dir = $BXAF_CONFIG['BXAF_DIR'] . $comp_subdir;
    $comparison_url = $BXAF_CONFIG['BXAF_URL'] . $comp_subdir;
...
if(! file_exists($comparison_dir . $combined_sorted_file_name)){
    ...
    $command = "/usr/bin/tail -n +2 {$comparison_dir}{$combined_file_name} | /usr/bin/sort -k4 -g -t\$'\\t' > {$comparison_dir}{$combined_sorted_file_name}";
    shell_exec($command);
}
...
if(! file_exists($comparison_dir . $combined_sorted_file_name)){
	header("Location: " . $BXAF_CONFIG['BXAF_APP_URL'] . "bxgenomics/tool_search/");
}

// $header = explode("\t", "GO Tree	TermID	Term	Enrichment	logP	Genes in Term	Target Genes in Term	Fraction of Targets in Term	Total Target Genes	Total Genes	Entrez Gene IDs	Gene Symbols	Raw_Pvalue");
...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
            ...
            var table = $('#myDataTable').DataTable({ 'pageLength': 10, 'lengthMenu': [[10, 100, 500, 1000], [10, 100, 500, 1000]], "order": [[3, 'asc']], dom: 'Blfrtip', buttons: ['colvis','copy','csv'] });
