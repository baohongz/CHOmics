...
<div class="container-fluid">
    ...
    <?php $help_key = 'WikiPathway View'; include_once( dirname(__DIR__) . "/help_content.php"); ?>
    ...
    <div class="my-3 w-100">
        <div class="form-group">
            ...
            <a href="Javascript: void(0);" class="hidden ml-2" id="btn_download_pathway" target="_blank">
              <i class="fas fa-link"></i> View Pathway Details
            </a>
        </div>
        <div class="form-group">
    ...
    <div class="w-100" id="div_form_upload_file">
        <form class="form-inline" id="form_upload_file" enctype="multipart/form-data">
            ...
            <input id="input_upload_file" type="file" class="mx-2" name="file" style="" onchange="$('#form_upload_file').submit();">
    ...
    <form class="my-2" id="form_pathway_chart" method="post">
        ...
        <div class="w-100 form-check form-check-inline">
            ...
            <button id="btn_submit" type='submit' class="btn btn-primary hidden"> View Pathway </button>
				...
				<?php
					...
					<table class="table table-bordered table-striped table-hover w-100" id="table_select_pathway">
                        ...
                        foreach ($BXAF_CONFIG['PATHWAY_LIST'] as $key => $value) {
                            $species = preg_match("/^Hs_/", $key) ? 'Human' : 'Mouse';
							echo '
							<tr>
                                <td>' . $species . ' &nbsp;</td>
								<td>' . $value . ' &nbsp;</td>
...
<script>
    ...
    $(document).ready(function() {
		...
		$(document).on('click', '.btn_select_similar_pathway', function() {
            ...
            $('#text_pathway_name').text((folder == 'mus_musculus' ? "(Mouse) " : "(Human) ") + displayed_name);

            $('#input_pathway').val(content);

            $('#modal_select_pathway').modal('hide');

            content = content.replace(/.+\_(WP\d+)\_\d+\.gpml/g, 'https://www.wikipathways.org/index.php/Pathway:$1');
            $('#btn_download_pathway').show().attr('href', content);

            if( $('.all_comparisons').length > 0 && $('#input_pathway').val() != ''){
        ...
        $(document).on('change', '#btn_use_demo_file', function() {
            ...
            if ($(this).is(':checked')) {
                ...
                var content = 'Hs_Abacavir_transport_and_metabolism_WP2712_86664.gpml';
        ...
        var options_generate_pathway_chart = {
            ...
            success: function(response){
                ...
                if (response.type == 'Error') {
                    ...
                    $('#results_pathway_diagram').attr('src', 'single_svg.php?time=' + response.time);
    ...
    function add_comparison_section(comparison_name) {
        ...
        var content = '<div class="my-3 table-light rounded border border-primary p-3" id="file_' + time + '" style="max-width: 1500px;">';
            ...
            content += '<div class="text-right">';
                // content += '<h3 class="">Data: <a target="_blank" href="' + url + '">' + comparison_name + '</a></h3>';
