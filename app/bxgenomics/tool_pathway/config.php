...
$BXAF_CONFIG['CURRENT_SYSTEM_CACHE'] = $BXAF_CONFIG['BXAF_DIR'] . "app_data/cache/tool_pathway/{$BXAF_CONFIG['BXAF_USER_CONTACT_ID']}/";
if (!is_dir($BXAF_CONFIG['CURRENT_SYSTEM_CACHE'])) mkdir($BXAF_CONFIG['CURRENT_SYSTEM_CACHE'], 0755, true);
$BXAF_CONFIG['CURRENT_SYSTEM_CACHE_URL'] = $BXAF_CONFIG['BXAF_URL'] . "app_data/cache/tool_pathway/{$BXAF_CONFIG['BXAF_USER_CONTACT_ID']}/";


// All Pathways
$BXAF_CONFIG['PATHWAY_LIST'] = array();

$sql = "SELECT `File`, `Name` FROM `tbl_wikipathways_info` WHERE `Species` LIKE '" . $_SESSION['SPECIES_DEFAULT'] . "%' AND `Type` IN ('Protein', 'GeneProduct', 'Rna') AND `DB_Name` IN ('Entrez Gene', 'Uniprot-TrEMBL', 'Ensembl')";
...
foreach($list as $row){
    ...
    if($_SESSION['SPECIES_DEFAULT'] == 'Human' && strpos($identifier, "hsa") === 0) $BXAF_CONFIG['KEGG_PATHWAY_LIST'][$identifier] = htmlspecialchars($name, ENT_QUOTES);
    else if($_SESSION['SPECIES_DEFAULT'] == 'Mouse' && strpos($identifier, "mmu") === 0) $BXAF_CONFIG['KEGG_PATHWAY_LIST'][$identifier] = htmlspecialchars($name, ENT_QUOTES);
}
...
if(!function_exists('find_similar_pathways')){
	function find_similar_pathways($pathway, $type = ''){
        ...
        if($type == 'reactome'){
            ...
            $BXAF_CONFIG['REACTOME_PATHWAY_LIST'] = unserialize( file_get_contents($pathway_file) );

            $pathways = $BXAF_CONFIG['REACTOME_PATHWAY_LIST'];
