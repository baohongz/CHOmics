...
if(isset($_GET['action']) && $_GET['action'] == "show_venn_diagram") {
	...
	$result_file_dir = $BXAF_CONFIG['BXAF_VENN_DATA_DIR'] . "result.csv";
	$result_file_url = $BXAF_CONFIG['BXAF_VENN_DATA_URL'] . "result.csv";
	...
	$sql = "SELECT * FROM `$db_table` WHERE `ID` IN (?a)";
...
// echo "_POST<pre>" . print_r($name_result, true) . "</pre>";
// echo "_POST<pre>" . print_r($value_result, true) . "</pre>";
// exit();
	...
	echo '
		<div class="row m-3" id="top_summary">
			...
			<div class="w-100 my-1"><a href="'. $result_file_url .'" class="btn btn-warning btn-sm" style="margin-bottom:10px;" download>Download Overlap Results</a></div>
	...
	<div class="card-header bg-primary">
		...
		Results
		<span class="pull-right" style="font-size:16px;"><a href="'. $result_file_url . '" style="color:#FFFF00;" download><i class="fas fa-hand-point-right fa-lg"></i> Download Results File</span>
		</h4>
	</div>
			...
			for($i=0; $i<count($value_result)-1; $i++){
				for($j=$i+1; $j<count($value_result); $j++){
					if(is_array($value_result[$i]) && is_array($value_result[$j]) && $intersect_number[$i]>0 && $intersect_number[$j]>0){
						//$temp_array = array_intersect($value_result[$i], $value_result[$j]);
...
if(isset($_GET['action']) && $_GET['action']=="save_gene_list"){
	...
	// $dataArray = array();
	//
	// $dataArray['User_ID'] 	= intval( $BXAF_CONFIG['BXAF_USER_CONTACT_ID'] );
	// $dataArray['Name'] 		= $_POST['Name'];
	// $dataArray['Category'] 	= $_POST['Category'];
	// $dataArray['Table'] 	= 'UserSavedLists';
