#updated Jan 9 2019 to handle empty homer output files
####GO plot
...
# system('cp /public/scripts/php/GO_Report/*.* .')
...
for (f in 1:length(folders) ) {
	...
	comp=str_extract(folder, "\\.\\./.+?/Downstream")
	...
	for (i in 1:length(list) ) {
		...
		#dedup for kegg
		if (nrow(data)>0) {  #only process ones with data
			if (list[i]=="kegg.txt") {
				data=data[!duplicated(data[, 2]), ]
			}
			N_top10=min(10, nrow(data))
			data2plot=data[N_top10:1, ] #choose what to plot, inverse order so first one on top
			plotGO(data2plot, titles[i], m)
			box("figure", col="grey")
			#combine all top10 data into a big table
			Comparison=rep(comp_title, N_top10)
			Category=rep(titles[i], N_top10)
			outdata=cbind(Comparison, Category, data[1:N_top10, ])

		if (N_all==0) {alldata=outdata; N_all=N_all+1}
		else{N_all=N_all+1; alldata=rbind(alldata, outdata) }
		}
	}
