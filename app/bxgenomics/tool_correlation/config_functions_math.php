...
//0: Pearson
//1: Spearman
function getCorrelationCoefficient($argOrg1 = array(), $argOrg2=array(), $type = 0){
	...
	if ($type == 0){
		...
		if($a2*$b2 > 0) return $axb / sqrt($a2*$b2);
		else return false;

	} elseif ($type == 1){

		return getCorrelationCoefficient_Spearman($argOrg1, $argOrg2);
...
function getCorrelationCoefficient_Spearman($argOrg1 = array(), $argOrg2=array()){
	...
	for ($i=0; $i < $length; $i++) {
		...
		$rank1 = $rankArray1[$i] + 1;  //array index stars from 0
...
function cleanTwoNumericArrays(&$array1, &$array2){
	...
	foreach($all_keys as $key){
		if (! array_key_exists($key, $array1) || ! is_numeric($array1[$key]) || ! array_key_exists($key, $array2) || ! is_numeric($array2[$key]) ){
			if (array_key_exists($key, $array1)) unset($array1[$key]);
			if (array_key_exists($key, $array2)) unset($array2[$key]);
		}
	}
