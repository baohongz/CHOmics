...
if (!is_dir($BXAF_CONFIG['USER_FILES']['TOOL_PCA'])) {
  mkdir($BXAF_CONFIG['USER_FILES']['TOOL_PCA'], 0755, true);
}

if (!is_dir($BXAF_CONFIG['USER_FILES']['TOOL_PCA'] . $BXAF_CONFIG['BXAF_USER_CONTACT_ID'])) {
  mkdir($BXAF_CONFIG['USER_FILES']['TOOL_PCA'] . $BXAF_CONFIG['BXAF_USER_CONTACT_ID'], 0755, true);
}
...
function get_PCA_dir($TIME_STAMP){
	...
	if (!file_exists($dir)){
		if ($TIME_STAMP != ''){
			$cmd = "{$find} {$BXAF_CONFIG['USER_FILES']['TOOL_PCA']} -name '{$TIME_STAMP}'";
			$dir = shell_exec($cmd);
		}
	}
