...
if (isset($_GET['action']) && trim($_GET['action']) == 'save_result') {
    ...
    if ($TIME_STAMP == 0) {
        $dir_from = $BXAF_CONFIG['USER_FILES']['TOOL_PCA'] . $BXAF_CONFIG['BXAF_USER_CONTACT_ID'];
    } else {
        $dir_from = $BXAF_CONFIG['USER_FILES']['TOOL_PCA'] . $BXAF_CONFIG['BXAF_USER_CONTACT_ID'] . '/' . $TIME_STAMP;
    }
    ...
    $rowid = $BXAF_MODULE_CONN -> insert($BXAF_CONFIG['TBL_BXGENOMICS_USERSAVEDRESULTS'], $info);

    $id_encrypted = bxaf_encrypt($rowid, $BXAF_CONFIG['BXAF_KEY']);
    $dir = $BXAF_CONFIG['USER_FILES']['TOOL_PCA'] . $BXAF_CONFIG['BXAF_USER_CONTACT_ID'] . '/' . $id_encrypted;
    if (! file_exists($dir)) {
        mkdir($dir, 0755, true);
    }

    foreach ($BXAF_CONFIG['PCA_R_FILE_LIST'] as $file) {
        copy( $dir_from . '/' . $file, $dir . '/' . $file );
    }
